...
class PetControllerTests {
    ...
    void testProcessCreationFormSuccess() throws Exception {
        mockMvc.perform(post("/owners/{ownerId}/pets/new", TEST_OWNER_ID)
                ...
                .andExpect(model().attribute("message", "New Pet has been Added")); // Check success message
    }

    @Test
    void testInitCreationFormOwnerNotFound() throws Exception {
    ...
    class ProcessCreationFormHasErrors {
        ...
        void testProcessCreationFormWithDuplicateName() throws Exception {
            mockMvc.perform(post("/owners/{ownerId}/pets/new", TEST_OWNER_ID)
                    ...
                    .andExpect(model().attributeHasErrors("pet"))
                    .andExpect(model().attributeHasFieldErrors("pet", "name"))
                    .andExpect(model().attributeHasFieldErrorCode("pet", "name", "duplicatePetName")) // Updated error code
                    .andExpect(status().isOk())
                    .andExpect(view().name("pets/createOrUpdatePetForm"));
        }
        ...
        void testProcessCreationFormWithInvalidBirthDate() throws Exception {
            ...
            mockMvc.perform(post("/owners/{ownerId}/pets/new", TEST_OWNER_ID)
                    ...
                    .andExpect(model().attributeHasFieldErrorCode("pet", "birthDate", "invalidBirthDate")) // Updated error code
                    .andExpect(status().isOk())
                    .andExpect(view().name("pets/createOrUpdatePetForm"));
        }
    ...
    void testProcessUpdateFormSuccess() throws Exception {
        mockMvc.perform(post("/owners/{ownerId}/pets/{petId}/edit", TEST_OWNER_ID, TEST_PET_ID)
                ...
                .andExpect(status().is3xxRedirection())
                .andExpect(view().name("redirect:/owners/" + TEST_OWNER_ID))
                .andExpect(model().attribute("message", "Pet details has been edited")); // Check success message
    }

    @Test
    void testInitUpdateFormOwnerNotFound() throws Exception {
        given(this.owners.findById(TEST_OWNER_ID)).willReturn(Optional.empty()); // Simulate owner not found
        mockMvc.perform(get("/owners/{ownerId}/pets/{petId}/edit", TEST_OWNER_ID, TEST_PET_ID))
                .andExpect(status().isNotFound())
                .andExpect(view().name("error"));
    ...
    @Test
    void testInitUpdateFormPetNotFound() throws Exception {
        Owner owner = new Owner();
        owner.setId(TEST_OWNER_ID);
        given(this.owners.findById(TEST_OWNER_ID)).willReturn(Optional.of(owner));
        given(this.owners.findPetById(999)).willReturn(Optional.empty());
